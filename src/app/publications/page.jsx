      ...
      <div className=" w-full max-w-5xl mx-auto p-5">
        <div className="w-full flex flex-col lg:flex-row gap-10">
          <div className="lg:w-1/3 w-full lg:sticky top-0 h-max pt-20">
            <div className="p-5 flex flex-col gap-5 border border-maincolor rounded-[10px] text-maincolor bg-white">
              <h1 className="font-semibold text-[20px] border-b border-maincolor transition-all cursor-pointer">
          ...
          <div className="lg:w-2/3 w-full flex flex-col gap-5">
            <div className="pt-20 -mb-10"  id="journal-articles">
              <div className="p-5 flex flex-col border border-maincolor rounded-[10px] text-maincolor">
                <h1 className="font-semibold text-[20px] border-b border-maincolor transition-all cursor-pointer">Journal Article</h1>
                {journalArticles.map((article, index) => (
                  <div key={index} className={cn("flex flex-col border-maincolor py-5", journalArticles.length !== index + 1 && "border-b")}>
            ...
            <div className="pt-20" id="conference-papers">
              <div className="p-5 flex flex-col border border-maincolor rounded-[10px] text-maincolor">
                <h1 className="font-semibold text-[20px] border-b border-maincolor transition-all cursor-pointer">Conference Paper</h1>
                {conferencePapers.map((article, index) => (
                  <div key={index} className={cn("flex flex-col border-maincolor py-5", conferencePapers.length !== index + 1 && "border-b")}>
                    <p className="text-gray-800"><span className="font-semibold">Year:</span> {article.year}</p>
                    <p className="font-semibold cursor-pointer">{article.title}</p>
                    {(article?.cite || article?.publisher) && <br />}
                    {article?.cite && <p className="text-gray-800"><span className="font-semibold">Cite this Research Publication:</span> {article?.cite}</p>}
